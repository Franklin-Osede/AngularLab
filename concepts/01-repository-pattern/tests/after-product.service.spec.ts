...
import { of } from "rxjs";
import { Product } from "../after/product-app/src/app/models/product.model";
import {
  IProductRepository,
  PRODUCT_REPOSITORY,
} from "../after/product-app/src/app/repositories/product.repository.interface";
import { ProductService } from "../after/product-app/src/app/services/product.service";
...
describe("ProductService (Repository Pattern - AFTER)", () => {
  ...
  describe("getProductsByCategory", () => {
    it("should get products by category from repository", () => {
      const mockProducts: Product[] = [];

      mockRepository.getByCategory.and.returnValue(of(mockProducts));

      service.getProductsByCategory("Electronics").subscribe((products) => {
        expect(products).toEqual(mockProducts);
      });

      expect(mockRepository.getByCategory).toHaveBeenCalledWith("Electronics");
    });
  });

  describe("getProductsInStock", () => {
    it("should get products in stock from repository", () => {
      const mockProducts: Product[] = [];

      mockRepository.getInStock.and.returnValue(of(mockProducts));

      service.getProductsInStock().subscribe((products) => {
        expect(products).toEqual(mockProducts);
      });

      expect(mockRepository.getInStock).toHaveBeenCalled();
    });
  });
  ...
  describe("getProductsByPriceRange", () => {
    it("should get products by price range via repository", () => {
      const mockProducts: Product[] = [];

      mockRepository.search.and.returnValue(of(mockProducts));

      service.getProductsByPriceRange(100, 500).subscribe((products) => {
        expect(products).toEqual(mockProducts);
      });
